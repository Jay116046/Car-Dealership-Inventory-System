...
const UserDashboard = () => {
  ...
  const handlePurchase = async (id) => {
    setPurchasingId(id);
    const result = await dispatch(purchaseVehicle(id));
    if (purchaseVehicle.fulfilled.match(result)) {
      toast.success('Vehicle purchased successfully!');
    } else {
      toast.error(result.payload || 'Failed to purchase vehicle');
    }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredVehicles.map((vehicle) => (
              <VehicleCard 
                ...
                isPurchasing={purchasingId === (vehicle._id || vehicle.id)}
              />
            ))}
          </div>
        )}
